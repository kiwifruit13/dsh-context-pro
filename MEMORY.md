...
| 2026-08-22 | **快照可选顶层字段 summary_sentence + progress** | 222.md/333.md 提案分析发现 333.md 是破坏性重写（英文键/统一对象），采用方案2：非阻塞纯追加，中文键/三态值/supersede 一字不改；prompt.ts 让 CoT 输出，ChainGraph 暴露 latestSummary/latestProgress，类型 SnapshotProgress |
| 2026-08-22 | **stripSnapshotLine 接线** | 设计存在但从未调用，JSON 行混入正文文本进 FilterSelector。hook eventToInput 内 `text=strip(rawText)` 入历史累积、`snapshot=extractSnapshot(rawText)` 从原文解析，剥离与解析分离 |
| 2026-08-22 | **操作链执行洞察 analyzeOperationProgress** | progress 停滞（current_step 连续≥3轮）→ warn/critical，blocker 非空 → 受阻 warn；归 gap-aggregation 类型；全局时间序列信号，不依赖图变更，`_snapshot?.progress` 存在即 always run |
| 2026-08-22 | **归因档案定位为推荐解释层** | confidenceProfile.rationale = 给模型的"为什么推荐这个方向"（"基于对话中的X，[洞察结论]（可信度Y）"），非独立第三产出；话题 basedOn 为结构化附件（节点内容+边关系+分数） |
| 2026-08-22 | **洞察模块架构** | 输入两路（剥离快照 + 按需历史 FilterSelector.query）；输出两路（getInsights 给模型 + getTopics 给用户）；全局视角非归因——归因是推荐解释层 |
...
| **发布包 cordis.patch.yml 自动应用机制** | 本项目 `cordis.patch.yml` + `package.json` dsh.bundle 配置 + `npm pack` 验证 | 任何 DSH 插件发布到 npm 后通过 `dsh plugin add` 一键安装 |


---

## 六、发布包 Cordis Patch 机制沉淀（2026-08-19）

### 自动应用全链路

| 步骤 | 触发动作 | 产物 |
|------|----------|------|
| 1 | 用户 `dsh plugin add @kiwifruit/dsh-context-pro` | CLI 下载 tarball |
| 2 | 解压包 → 读取根目录 `cordis.patch.yml` | 包内 patch 内容 |
| 3 | 合并到 `$DSH_HOME/.dsh/profiles/<name>/cordis.patch.yml` | Profile patch 更新 |
| 4 | 触发 HMR 热重载 | 插件即时激活，无需重启 |

### 包内 patch 内容（发布时随包分发）

```yaml
# cordis.patch.yml（包根目录）
- insert:
    - id: context-pro
      name: '@kiwifruit/dsh-context-pro'   # 必须是 npm 包名，非 file:///
      config:
        chains:
          enabled: true
          injectProtocol: true
...
| 配置项 | 状态 | 证据 |
|--------|------|------|
| 包内含 `cordis.patch.yml` | ✅ | `npm pack --dry-run` 列表含 `435B cordis.patch.yml` |
| `files` 数组含该文件 | ✅ | `package.json: "files": [..., "cordis.patch.yml", ...]` |
| `dsh.bundle.patch` 指向正确 | ✅ | `"patch": "./cordis.patch.yml"` |
| `name` 用 npm 包名 | ✅ | `@kiwifruit/dsh-context-pro` |
| 默认配置开箱即用 | ✅ | `chains.enabled: true` 等 |

### 避坑复用：发布包常见坑

| 坑 | 症状 | 规避 |
|----|------|------|
| 包内无 `cordis.patch.yml` | `dsh plugin add` 后不生效 | `npm pack --dry-run` 必看 tarball 列表 |
| `files` 漏包 patch 文件 | 发布后 tarball 无该文件 | `package.json files` 必含 `cordis.patch.yml` |
| `name` 写 `file:///` | 用户装后加载本地路径报错 | 发布包 patch 必须用 npm 包名 |
| `config` 字段名拼写错 | 装了不生效 | 对照 `Config` schema 核对 |
| 同名 `id` 冲突 | 其它包也用 `context-pro` | 用带命名空间的 id（如 `@kiwifruit/context-pro`） |
...
# 1. 打包干跑
npm pack --dry-run
# 必见：cordis.patch.yml 在列表中

# 2. 解包实测
npm pack && tar -tzf *.tgz | grep cordis.patch.yml
# 必见：package/cordis.patch.yml

# 3. 本地安装验证
cd /tmp && npm install /path/to/kiwifruit-dsh-context-pro-0.3.3.tgz
# 必见：node_modules/@kiwifruit/dsh-context-pro/cordis.patch.yml 存在
```

---

## 七、Chip 话题推荐重构记录（2026-08-19）

### 背景
原 `Card` 卡片太重、点击复制需手动粘贴、竖向堆叠占空间。参考 `ship/` 文件夹 Chip 设计，改为轻量胶囊按钮。

### 架构调整

```
Insight Engine（超然层）→ HTTP/SSE → Chip Client（host.call 发送消息）
```

### 核心改动（`src/client.js` 全量重写，329 行 → 189 行）

| 特性 | 实现 |
|------|------|
| **视觉** | 横向胶囊（`border-radius: 999px`），最多 3 条 |
| **相关性评分** | 严重度 35% + 证据 25% + 时效 20% + 链活跃 15% + 类型多样 5% |
| **Top-3 选取** | 基础分降序 → 同 kind 惩罚 0.15 → 取前 3 正分 |
| **实时更新** | SSE `/topics/stream` 订阅，话题变化即时 `setChips` |
| **发送消息** | 1) `host.call('conversation.send', {text})` 2) `fetch /api/conversation/message` 3) `clipboard.writeText()` |
| **零话题零渲染** | CSS `:empty {display:none}` |
| **类型区分** | `.kind-convergence` 蓝色 / `.kind-extension` 紫色 |

### 版本发布
- `v0.3.3` → npm 上线，含新 Chip 实现
- 全验证通过：e2e PASS / verify-chains 40/40 / verify-protocol 26/26 / cordis-config 122/122
